      ...
      <v-row>
        <v-btn block @click="saveSample">
          <v-progress-circular
            ...
            color=""
          ></v-progress-circular>
          <span v-else>{{ $t("labels.action.saveSample") }}</span>
        </v-btn>
      </v-row>
    </v-col>
  </v-container>
...
export default {
  ...
  methods: {
    async saveSample() {
      await this.addSpecimen(this.$data);
      this.resetData();
    ...
    resetData() {
      ...
      this.maturity = "";
      this.tagType = "";
      this.preCaudalLength = 0;
      this.finLength = 0;
      this.tailLength = 0;
      this.standardLength = 0;
      this.girth = 0;
      this.tagLocation.lat = 24.786734541988906;
      this.tagLocation.lng = -81.60644531250001;
      this.species = "";
      this.sampleType = "";
