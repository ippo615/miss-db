<template>
  <v-container>
    <v-col cols="12">
      <v-row>
        ...
        >
          <template v-slot:selection="data">
            ...
            >
              <v-list-item-title>{{
                data.item.displayCommonName
              }}</v-list-item-title>
              <v-list-item-subtitle style="opacity: 0.5"
                >{{ data.item.order }} / {{ data.item.family }} /
                {{ data.item.genus }} /
                {{ data.item.species }}</v-list-item-subtitle
              >
      ...
      <v-row v-if="!loadingLocation">
        <location-input
          @update:lat="onUpdateLat"
          @update:lng="onUpdateLng"
          :initialLocation="tagLocation"
        />
      </v-row>
      ...
      <v-row>
        <v-btn block @click="editSample">
          <v-progress-circular
            ...
            color=""
          ></v-progress-circular>
          <span v-else>{{ $t("labels.action.editSample") }}</span>
        </v-btn>
      </v-row>
    </v-col>
  </v-container>
...
export default {
  ...
  mounted() {
    ...
    let doc = this.getSpecimen.find((data) => data.docId == docId);
    console.log(doc, "single");
    this.sex = doc.sex;
    this.maturity = doc.maturity;
    this.tagType = doc.tagType;
    this.preCaudalLength = doc.preCaudalLength;
    this.finLength = doc.finLength;
    this.tailLength = doc.tailLength;
    this.standardLength = doc.standardLength;
    this.girth = doc.girth;
    setTimeout(() => {
      this.tagLocation.lat = doc.tagLocation.lat;
      this.tagLocation.lng = doc.tagLocation.lng;
      this.loadingLocation = false;
    }, 3000);
    this.species = doc.species;
    this.sampleType = doc.sampleType;
    this.releaseCondition = doc.releaseCondition;
    this.hookTime = doc.hookTime;
    this.soakTime = doc.soakTime;
    this.depth = doc.depth;
    this.tide = doc.tide;
    this.salinity = doc.salinity;
    this.temperature = doc.temperature;
    this.extendedInfo = doc.extendedInfo;
  },
  computed: {
    items() {
      return {
    ...
    sharkSpecies() {
      ...
      for (let i = 0, l = sharks.length; i < l; i += 1) {
        ...
        let names = [shark.order, shark.family, shark.genus, shark.species];
        ...
        shark.displayCommonName = shark.common_name.replace(
          /(^|\s)\S/g,
          function (t) {
  ...
  methods: {
    async editSample() {
      let docId = this.$route.params.id;
      let payload = { id: docId, data: this.$data };
      await this.editSpecimen(payload);
      this.$router.push({ path: "/samples" });
    },
    onUpdateLat(lat) {
